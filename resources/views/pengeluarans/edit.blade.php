<x-layouts.app>
    <div class="mb-6 flex items-center text-sm">
        ...
        <a href="{{ route('pengeluarans.index') }}" class="text-blue-600 dark:text-blue-400 hover:underline">{{ $title ?? __('Pengeluarans') }}</a>
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mx-2 text-gray-400" fill="none" viewBox="0 0 24 24"
            stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
        </svg>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="p-6 min-h-[500px]">
            <form action="{{ route('pengeluarans.update', $data) }}" method="POST" class="max-w-3xl">
                @csrf
                @method('PUT')
                ...
                <div class="mb-5">
                    <x-forms.input label="{{ $column['title'] }}" name="{{ $column['name'] }}" type="date" value="{{ old($column['name'], $data->{$column['name']}) }}" required />
                </div>
                ...
                <div class="mb-5">
                    <x-forms.input label="{{ $column['title'] }}" name="{{ $column['name'] }}" type="number" step="{{ $column['name'] === 'nominal' ? '1' : '0.01' }}" value="{{ old($column['name'], $data->{$column['name']}) }}" />
                </div>
                ...
                <div class="mb-5">
                    ...
                    <select
                        ...
                        @if(old($column['name'], $data->{$column['name']}) == $option['value'])
                        <option value="{{ $option['value'] }}" selected>
                            {{ $option['label'] }}
                        </option>
                        @else
                        <option value="{{ $option['value'] }}" {{ old($column['name'], $data->{$column['name']}) == $option['value'] ? 'selected' : '' }}>
                            {{ $option['label'] }}
                        </option>
                        @endif
                        @endforeach
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (productSelect) {
                const choices = new Choices(productSelect, {
                    ...
                    placeholder: true,
                    placeholderValue: 'Cari atau Pilih Customer',
                    shouldSort: false,
                    classNames: {
