<x-layouts.app>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="p-6 min-h-[500px]">
            <form action="{{ route('pengeluarans.store') }}" method="POST" class="max-w-3xl">
                @csrf
                @method('POST')
                ...
                <div class="mb-5">
                    <x-forms.input label="{{ $column['title'] }}" name="{{ $column['name'] }}" type="date" value="{{ old($column['name']) }}" required />
                </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (productSelect) {
                const choices = new Choices(productSelect, {
                    ...
                    placeholder: true,
                    placeholderValue: 'Cari atau Pilih Kategori',
                    shouldSort: false,
                    classNames: {
